...
export default function useSendFileMessageCallback(
  ...
  const sendMessage = useCallback(
    (file) => {
      ...
      const createCustomParams =
        onBeforeSendFileMessage &&
        typeof onBeforeSendFileMessage === 'function';

      const compressibleFileType =
        file.type === 'image/jpg' ||
        file.type === 'image/png' ||
        file.type === 'image/jpeg';
      const compressibleRatio = compressionRate > 0 && compressionRate < 1;
      // pxToNumber returns null if values are invalid
      const compressibleDiamensions =
        utils.pxToNumber(resizingWidth) || utils.pxToNumber(resizingHeight);

      const canCompressImage =
        compressibleFileType && (compressibleRatio || compressibleDiamensions);
      ...
      if (canCompressImage) {
            ...
            context.canvas.toBlob(
              (newImageBlob) => {
                ...
                if (createCustomParams) {
                  logger.info(
                    'Channel: Creating params using onBeforeSendFileMessage',
                    onBeforeSendFileMessage
                  );
                }
                ...
                const pendingMessage = currentGroupChannel.sendFileMessage(
                  ...
                  (response, err) => {
                    ...
                    const [message, error] = swapParams
                      ? [err, response]
                      : [response, err];
                    if (error) {
                    ...
                    logger.info(
                      'Channel: Sending file message success!',
                      message
                    );
                    messagesDispatcher({
        ...
        const pendingMsg = currentGroupChannel.sendFileMessage(
          params,
          (response, err) => {
            const swapParams = sdk.getErrorFirstCallback();
            const [message, error] = swapParams
