...
describe('Label', () => {
  it('should have default classname if props are not provided', function () {
    ...
    expect(
      component
        .find('.sendbird-label')
        .hasClass('sendbird-label--color-onbackground-1')
    ).toBe(true);
    expect(component.find('.sendbird-label').text()).toEqual(text);
  });
  ...
  it('should have each typography', function () {
    for (let color in LabelColors) {
      for (let typography in LabelTypography) {
        const component = shallow(
          <Label color={color} type={typography}>
            Component
          </Label>
        );
        expect(
          component
            .find('.sendbird-label')
            .hasClass(changeColorToClassName(color))
        ).toBeTruthy();
        expect(
          component
            .find('.sendbird-label')
            .hasClass(changeTypographyToClassName(typography))
        ).toBe(true);
      }
    }
  });
